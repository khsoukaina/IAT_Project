...
while not mdp_instance.is_terminal(state):
    ...
    plt.clf()  # Nettoyez la figure courante pour préparer la nouvelle visualisation
    mdp_instance.visualise(agent_position=state)  # Visualisez l'état actuel avec la position de l'agent
    plt.draw()  # Mettez à jour la fenêtre graphique avec le contenu dessiné
